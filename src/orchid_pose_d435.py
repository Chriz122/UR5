...
def orchid_pose_seg_area_leafs_number_predict_d435(img, depth_frame, pose_model_name, seg_model_name, predict_pose_number):
    ...
    if results.boxes.data.tolist() is None or results.boxes.id is None:
        ...
        keypoints = results.keypoints.cpu().numpy()
        # Prepare CSV data
        csv_data = [["id", "(x0, y0)", "(x1, y1)", "(x2, y2)", "(x3, y3)", "(x4, y4)", "Leafs-Number", "Leafs-Area", "Soil-Angle", "Rotate-Angle"]]
        ...
        for obj, keypoint, id in zip(boxes, keypoints.data, ids):
            ...
            if orchid_RGB(img_seg_copy, [nrow[2][0], nrow[2][1]]) and orchid_RGB(img_seg_copy, [nrow[3][0], nrow[3][1]]):
                ...
                needed_leafs = check_point_to_points([nrow[1][0], nrow[1][1]], results_row_leafs_seg,
                                                     [nrow[4][0], nrow[4][1]], [nrow[5][0], nrow[5][1]])
                # print(needed_leafs)
                ...
                if angle_rotate != 0 and (abs(angle_rotate) >= 10):
                    ...
                    # Line from (x3, y3) to (x4, y4)
                    cv2.line(img_seg, (x3, y3), (x4, y4), (255, 0, 0), 2)
                    
                    # Line from (x3, y3) to (x4, y4)
                    cv2.line(img_seg, (x1, y1), (x2, y2), (255, 255, 0), 2)
            
                    # Vertical line through the center of the bounding box
                    cv2.line(img_seg, (left, y0), (right, y0), (0, 0, 255), 2)
            
                    # Draw the angle text
                    cv2.putText(img_seg, f'{int(angle_soil)} {int(angle_rotate)} deg', (center_x + 10, center_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
...
def orchid_pose_seg_area_predict_d435_new(img, depth_frame, pose_model_name, seg_model_name, predict_pose_number):
    ...
    if results.boxes.data.tolist() is None or results.boxes.id is None:
        ...
        for obj, keypoint, id in zip(boxes, keypoints.data, ids):
            ...
            if orchid_RGB(img_seg_copy, [nrow[4][0], nrow[4][1]]) and orchid_RGB(img_seg_copy, [nrow[5][0], nrow[5][1]]):
    ...
    if results.boxes.data.tolist() is None or results.boxes.id is None:
        ...
        for obj, keypoint, id in zip(boxes, keypoints.data, ids):
            ...
            if orchid_RGB(img_seg_copy, [nrow[4][0], nrow[4][1]]) and orchid_RGB(img_seg_copy, [nrow[5][0], nrow[5][1]]):
                ...
                needed_leafs = check_point_to_points([nrow[1][0], nrow[1][1]], results_row_leafs_seg,
                                                     [nrow[2][0], nrow[2][1]], [nrow[3][0], nrow[3][1]])
                # print(needed_leafs)
                results_rows.append(needed_leafs)
                row.append(needed_leafs[1])
                row.append(needed_leafs[0])
                ...
                if angle_rotate != 0 and (abs(angle_rotate) >= 10):
                    ...
                    # Vertical line through the center of the bounding box
                    cv2.line(img_copy, (left, y0), (right, y0), (0, 0, 255), 2)
            
                    # Draw the angle text
                    cv2.putText(img_copy, f'{int(angle_soil)} {int(angle_rotate)} deg', (center_x + 10, center_y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                ...
                for i, sk in enumerate(skeleton):
                    ...
                    cv2.line(img_copy, pos1, pos2, [int(x) for x in limb_color[i]], thickness=2, lineType=cv2.LINE_AA)
