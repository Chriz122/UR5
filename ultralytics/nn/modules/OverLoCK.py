...
Modified to remove mmengine dependency, align with EMO's usage pattern for backbones,
and return a list of four feature maps for downstream detection tasks.
'''
import torch
import torch.hub
import timm
...
class OverLoCK(nn.Module):
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0,
                 norm_layer=LayerNorm2d,
                 num_classes=1000, # Kept for loading pretrained weights, but not used in forward
        ...
        # The following modules are part of the original classification architecture
        # but are not used in the modified forward pass for feature extraction.
        # They are kept for weight loading compatibility.
        ...
        # The sub_blocks are not used in the backbone forward pass but are initialized
        # to allow loading the full pretrained model weights.
        ...
        # The classification head is an identity layer for backbone usage.
        ...
        # Add self.width_list for compatibility with detection frameworks
        # Note: Set model to eval mode for this dummy forward pass
        self.eval()
        self.width_list = [i.shape[1] for i in self.forward(torch.randn(1, 3, 224, 224))]
        self.train()
    ...
    def forward(self, x):
        """
        Modified forward pass to return a list of four feature maps for use as a backbone.
        This provides multi-scale features for downstream tasks like object detection.
        """
        outputs = []
...
def _load_pretrained(model, url):
    ...
    try:
        ...
        # Find the actual model weights, which might be nested under a 'model' key
        if 'model' in checkpoint:
            checkpoint = checkpoint['model']
        # The pretrained weights are for classification, so we use strict=False
        # to ignore keys that don't match (e.g., the final classifier head).
...
if __name__ == '__main__':
    ...
    # Set `pretrained=True` to load ImageNet-1k weights
    model = overlock_xt(pretrained=True).to(device)
    model.eval()

    print(f"Model created. Backbone output channel widths (width_list): {model.width_list}")

    # Test forward pass with a dummy tensor
    x = torch.randn(1, 3, 640, 640).to(device)
    y_list = model(x)
    print("\n--- Output before reparametrization ---")
    print("Forward pass returned a list of {} tensors.".format(len(y_list)))
    print("Shapes of output tensors:", [t.shape for t in y_list])

    # Reparametrize the model
    # More details can be found at: https://github.com/AILab-CVC/UniRepLKNet/tree/main
    print("\nReparametrizing model...")
    model.reparm()
    model.eval()
    print("Reparametrization complete.")

    # Test forward pass after reparametrization
    z_list = model(x)
    print("\n--- Output after reparametrization ---")
    print("Shapes of output tensors:", [t.shape for t in z_list])

    # Showing difference between original and reparametrized model
    total_diff = sum([(z - y).abs().sum() for z, y in zip(z_list, y_list)])
    print(f"\nTotal absolute difference between outputs: {total_diff.item()}")
    if total_diff.item() < 1e-4:
        print("The difference is negligible, reparametrization is successful.")
    else:
        print("The difference is significant, please check the reparametrization logic.")