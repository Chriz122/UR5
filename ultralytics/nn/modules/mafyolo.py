...
class DepthBottleneckUni(nn.Module):
    def __init__(self,
        ...
        if use_depthwise:
            ...
            self.conv2 = UniRepLKNetBlock_pro(mid_channel, kernel_size=kersize)
            self.act = nn.SiLU()
...
class UniRepLKNetBlock_pro(nn.Module):
...
class DepthBottleneckUniv2(nn.Module):
    def __init__(self,
        ...
        if use_depthwise:
            ...
            self.conv3 = UniRepLKNetBlock_pro(mid_channel2, kernel_size=kersize)
            self.act1 = nn.SiLU()
