...
# from timm.models.registry import register_model  # Not using timm registration anymore
# from timm.models.vision_transformer import _cfg # Not using timm _cfg anymore
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # MODIFIED: This forward function now ONLY applies the blocks for its stage.
        # The downsampling is handled in the main MALA.forward loop.
        _, _, h, w = x.size()
...
class MALA(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, img_size=224,
        ...
        # --- Classification head (kept for potential use, but not used in backbone forward) ---
        ...
        # --- Add width_list calculation (copied from SMT) ---
        self.width_list = []
        try:
            self.eval() 
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        """
        Forward pass modified to return a list of feature maps from each stage.
        This is required for use as a backbone in detection/segmentation models.
        """
        # MODIFIED: The logic is changed to separate feature extraction and downsampling.
        feature_outputs = []
        x = self.patch_embed(x)

        for layer in self.layers:
            # Pass through the blocks of the current stage
            x = layer(x)
            # Append the feature map of the current stage BEFORE downsampling
            feature_outputs.append(x)
            
            # If a downsampler exists in this layer, apply it to prepare 'x' for the next stage
            if layer.downsample is not None:
                x = layer.downsample(x)
            
        return feature_outputs

# --- Factory Functions (modified to match SMT style) ---

def MALA_T(pretrained: bool = False, img_size: int = 224, **kwargs):
    model = MALA(
        img_size=img_size,
        ...
        flagss=[['l']*2, ['l']*2, ['v', 'v']*3, ['v']*2], # Flags updated to match depths
        **kwargs
    )
    return model

def MALA_S(pretrained: bool = False, img_size: int = 224, **kwargs):
    model = MALA(
        img_size=img_size,
        ...
        layerscales=[True, True, True, True],
        layer_init_values=[1, 1, 1, 1],
        flagss=[['l']*3, ['l']*5, ['v', 'v']*5, ['v']*3], # Flags updated to match depths
        **kwargs
    )
    return model

def MALA_B(pretrained: bool = False, img_size: int = 224, **kwargs):
    model = MALA(
        img_size=img_size,
        ...
        flagss=[['l']*4, ['l']*6, ['v', 'v']*6, ['v']*6], # Flags updated to match depths
        **kwargs
    )
    return model

def MALA_L(pretrained: bool = False, img_size: int = 224, **kwargs):
    model = MALA(
        img_size=img_size,
        ...
        flagss=[['l']*4, ['l']*7, ['v', 'v']*10, ['v']*8], # Flags updated to match depths
        **kwargs
    )
    return model
...
if __name__ == '__main__':
    # Example of how to use the new models
    img_h, img_w = 224, 224
    
    print("--- Creating MALA-T model ---")
    model = MALA_T(img_size=img_h)
    print("Model created successfully.")
    
    # Check the calculated width_list
    print("Calculated width_list:", model.width_list)

    # Test forward pass
    input_tensor = torch.rand(2, 3, img_h, img_w)
    print(f"\n--- Testing MALA-T forward pass (Input: {input_tensor.shape}) ---")
    ...
    try:
        ...
        for i, features in enumerate(output_features):
            print(f"Stage {i+1}: {features.shape}") # Should be [B, C_i, H_i, W_i]
