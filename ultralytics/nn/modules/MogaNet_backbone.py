...
__all__ = ['MogaNet_Xtiny', 'MogaNet_Tiny', 'MogaNet_Small', 'MogaNet_Base', 'MogaNet_Large', 'MogaNet_Xlarge']
...
class MogaNet(nn.Module):
    ...
    def __init__(self,
        ...
        # Add width_list for compatibility with detection frameworks
        # Set model to evaluation mode for the dummy forward pass
        self.eval()
    ...
    def forward(self, x):
        ...
        # Always return a list of features for dense prediction tasks
        return outs
...
if __name__ == '__main__':
    # --- 測試1: 不使用預訓練權重 ---
    print("### Testing MogaNet without pre-trained weights ###")
    
    # 建立模型
    model_no_pretrain = MogaNet_Tiny()
    model_no_pretrain.eval()
    
    # 打印 width_list
    print(f"Generated width_list: {model_no_pretrain.width_list}")
    
    # 建立 dummy 輸入
    dummy_input = torch.randn(1, 3, 640, 640)
    
    # 進行前向傳播
    output_no_pretrain = model_no_pretrain(dummy_input)
    
    # 打印輸出資訊
    print("\nOutput is a list of tensors. Shapes:")
    for i, out in enumerate(output_no_pretrain):
        print(f"  Stage {i+1}: {out.shape}")

    print("\n" + "="*60 + "\n")

    # --- 測試2: 使用預訓練權重 ---
    print("### Testing MogaNet with pre-trained weights ###")
    
    # 建立模型並載入權重
    try:
        model_with_pretrain = MogaNet_Tiny(pretrained=True)
        model_with_pretrain.eval()
        print("Successfully created model and loaded pre-trained weights.")

        # 打印 width_list
        print(f"Generated width_list: {model_with_pretrain.width_list}")

        # 進行前向傳播
        output_with_pretrain = model_with_pretrain(dummy_input)

        # 打印輸出資訊
        print("\nOutput is a list of tensors. Shapes:")
        for i, out in enumerate(output_with_pretrain):
            print(f"  Stage {i+1}: {out.shape}")
        
        # 驗證權重是否不同
        # 由於初始化和預訓練權重的不同，最後一層的輸出應該不一致
        are_outputs_same = torch.allclose(output_no_pretrain[-1], output_with_pretrain[-1])
        print(f"\nOutputs from random vs. pretrained models are the same: {are_outputs_same}")

    except Exception as e:
        print(f"Could not load pre-trained weights. Error: {e}")
        print("This might be due to a network issue or the URL being temporarily unavailable.")