...
__all__ = ['MobileNetV3_large_n', 'MobileNetV3_large_s', 'MobileNetV3_large_m', 'MobileNetV3_small_n', 'MobileNetV3_small_s', 'MobileNetV3_small_m']
...
class MobileNetV3(nn.Module):
    def __init__(
        ...
        layers = [
            ConvNormActivationBlock(
                input_channel, int(16 * depth_multiplier), (3, 3), stride=2, padding=1, activation=nn.Hardswish
            )
        ]
        ...
        # 根据配置和 depth_multiplier 放缩 hidden_channels 和 _out_channel
        hidden_channels = int((576 if config == "small" else 960) * depth_multiplier)
        _out_channel = int((1024 if config == "small" else 1280) * depth_multiplier)
 
        # 最后添加一个 ConvNormActivationBlock 层
        layers.append(
            ConvNormActivationBlock(
                int(out_channels * depth_multiplier),  # 确保通道数放缩
                hidden_channels,
                kernel_size=(1, 1),
                bias=False,
...
def MobileNetV3_small_n():
    model = MobileNetV3(config='small',depth_multiplier=0.25)
    return model
 
def MobileNetV3_small_s():
    model = MobileNetV3(config='small',depth_multiplier=0.5)
    return model
 
def MobileNetV3_small_m():
    model = MobileNetV3(config='small',depth_multiplier=1)
    return model
 
def MobileNetV3_large_n():
    model = MobileNetV3(config='large',depth_multiplier=0.25)
    return model
 
def MobileNetV3_large_s():
    model = MobileNetV3(config='large',depth_multiplier=0.5)
    return model
 
def MobileNetV3_large_m():
    model = MobileNetV3(config='large',depth_multiplier=1)
    return model
...
if __name__ == "__main__":
    ...
    # Model
    mobilenet_v3 = MobileNetV3_large_M()
