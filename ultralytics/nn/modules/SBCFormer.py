...
class Attention(torch.nn.Module):
    ...
    def forward(self, x, h, w):
        """
        Modified forward to accept h, w explicitly.
        This handles rectangular inputs correctly.
        """
        B, N, C = x.shape
        
        # Use passed h, w instead of sqrt(N)
        ...
        # Dynamic bias interpolation
        # If N matches the resolution^2, we can use the standard path
        # But for robustness with rectangular inputs, we check shape match
        if self.training:
            bias_idx = self.attention_bias_idxs
            bias_val = self.attention_biases
            bias = bias_val[:, bias_idx]
        else:
            bias = self.ab

        # If current N does not match the cached bias shape (N_res, N_res)
        if bias.shape[-1] != N:
            # Interpolate bias to (Heads, N, N)
            # This is a heuristic to adapt the position bias to new spatial dimensions
            bias = F.interpolate(bias.unsqueeze(0), size=(N, N), mode='bicubic', align_corners=False)
            bias = bias.squeeze(0)
...
class SBCFormerBlock(nn.Module):   # building block
    def __init__(self, depth_invres, depth_mattn, depth_mixer, dim, key_dim, num_heads, mlp_ratio=4., attn_ratio=2, drop=0., attn_drop=0.,
        ...
        # Change to ModuleList to iterate and pass extra args (h, w)
        self.trans_blocks = nn.ModuleList()
        for k in range(self.depth_mattn):
            self.trans_blocks.append(ModifiedTransformer(dim=dim, key_dim=key_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, attn_ratio=attn_ratio,
    ...
    def forward(self, x):
        ...
        # Capture pooled dimensions accurately (crucial for rectangular inference)
        _, _, h_pooled, w_pooled = x.shape
        
        x = self.mixer(x)
        x = x.flatten(2).transpose(1, 2) # (B, N, C)
        
        # Iterate manually to pass h_pooled, w_pooled
        for blk in self.trans_blocks:
            x = blk(x, h_pooled, w_pooled)
        
        # Reshape using the known pooled dimensions
        x = x.transpose(1, 2).reshape(B, C, h_pooled, w_pooled)
...
class SBCFormer(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[128,320,512], key_dim=32, num_heads=[2,4,8], attn_ratio=2, mlp_ratio=4, invres_ratio=2,
        ...
        # Initialize width_list
        dummy_in = torch.randn(1, in_chans, img_size, img_size) 
        self.width_list = [i.size(1) for i in self.forward(dummy_in)]
    ...
    def forward(self, x):
        ...
        # Stage 0
        x = self.merging_blocks[0](x) # Stem
        outs.append(x)                # Output 0
        
        x = self.sbcformer_blocks[0](x) 
        outs.append(x)                # Output 1

        # Stage 1
        x = self.merging_blocks[1](x)
        x = self.sbcformer_blocks[1](x)
        outs.append(x)                # Output 2
            
        # Stage 2
        x = self.merging_blocks[2](x)
        x = self.sbcformer_blocks[2](x)
        outs.append(x)                # Output 3
...
@register_model
def sbcformer_xs(pretrained=None, frozen_stages=0, distillation=False, fuse=False, pretrained_cfg=None,
                 model_cfg=SBCFormer_xs_cfg, **kwargs):
    model = SBCFormer(**model_cfg, **kwargs)
    if pretrained:
...
@register_model
def sbcformer_s(pretrained=None, frozen_stages=0, distillation=False, fuse=False, pretrained_cfg=None,
                model_cfg=SBCFormer_s_cfg, **kwargs):
    model = SBCFormer(**model_cfg, **kwargs)
    if pretrained:
        state_dict = torch.load(pretrained, map_location='cpu')
        if 'model' in state_dict:
...
@register_model
def sbcformer_b(pretrained=None, frozen_stages=0, distillation=False, fuse=False, pretrained_cfg=None,
                model_cfg=SBCFormer_b_cfg, **kwargs):
    model = SBCFormer(**model_cfg, **kwargs)
    if pretrained:
        state_dict = torch.load(pretrained, map_location='cpu')
        if 'model' in state_dict:
...
@register_model
def sbcformer_l(pretrained=None, frozen_stages=0, distillation=False, fuse=False, pretrained_cfg=None,
                model_cfg=SBCFormer_l_cfg, **kwargs):
    model = SBCFormer(**model_cfg, **kwargs)
    if pretrained:
        state_dict = torch.load(pretrained, map_location='cpu')
        if 'model' in state_dict:
...
if __name__ == "__main__":
    # Test to ensure 640x640 input also works now (Dynamic Attention)
    model = SBCFormer_XS()
    dummy_input = torch.randn(1, 3, 640, 640)
    features = model(dummy_input)
    
    print(f"Model Width List: {model.width_list}")
