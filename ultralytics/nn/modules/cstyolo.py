...
def window_partition(x, window_size: int):
    """
    # Feature map is divided into one without overlapping windows in term of window_size.
    Args:
        x: (B, H, W, C)
...
def window_reverse(windows, window_size: int, H: int, W: int):
    """
    # Every window reverted to one feature map.
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size(M)
...
class SwinTransformerLayer(nn.Module):
    ...
    def create_mask(self, x, H, W):
        ...
        # Hp and Wp are integer multiples of window_size
        Hp = int(np.ceil(H / self.window_size)) * self.window_size
        Wp = int(np.ceil(W / self.window_size)) * self.window_size
        # The channels are arranged in the same order as the feature map to facilitate subsequent window_partitions
...
class TridentBlock(nn.Module):
    def __init__(self, c1, c2, stride=1,  e=1, padding=[1, 2, 3], dilate=[1, 2, 3], bias=False):
        ...
        self.dilate = dilate
        self.share_weightconv1 = nn.Parameter(torch.Tensor(c_, c_, 1, 1))
...
class FEEM(nn.Module):
    def __init__(self, c1, c2,  stride=1):
        ...
        self.layer = TridentBlock(c1, c2, stride=stride,   e=1)
...
class Aatten(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            out = self.sigmoid(y3)
            out2 = out*y3
            result = out2[:,:self.c_,...] + out2[:,self.c_:self.c_*2,...] + out2[:,self.c_*2:self.c_*3,...]+ out2[:,self.c_*3:self.c_*4,...]
            result = self.conv2(result) + residual
...
class MyConcat6(nn.Module):
    ...
    def forward(self, x):
        w = self.w
        weight = w / (torch.sum(w, dim=0) + self.epsilon)  # normalized weights
        x = [weight[0] * x[0], weight[1] * x[1], weight[2] * x[2], weight[3] * x[3], weight[4] * x[4], weight[5] * x[5]]
        return torch.cat(x, self.d)
...
##### end of novel modules in cst-yolo ####