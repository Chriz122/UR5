...
This version is modified to be compatible with frameworks like ultralytics,
by returning a list of feature maps from its forward pass.
"""
import math
import logging
...
class InceptionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None, depths=None,
        ...
        # Stage 1
        self.patch_embed1 = FirstPatchEmbed(in_chans=in_chans, embed_dim=embed_dims[0])
        ...
        # Classification head (kept for standalone use, but not used in feature extraction mode)
    ...
    def forward(self, x):
        # As a feature extractor, it returns a list of feature maps.
...
def iformer_small(pretrained=False, img_size=224, **kwargs):
    """
    Inception Transformer-Small model
    """
    depths = [3, 3, 9, 3]
    ...
    # Choose config based on img_size
    if img_size == 384:
...
def iformer_base(pretrained=False, img_size=224, **kwargs):
    """
    Inception Transformer-Base model
    """
    depths = [4, 6, 14, 6]
...
def iformer_large(pretrained=False, img_size=224, **kwargs):
    """
    Inception Transformer-Large model
    """   
    depths = [4, 6, 18, 8]
...
if __name__ == '__main__':
    img_h, img_w = 640, 640
    print("--- 測試 Inception Transformer Small (iFormer-S) 模型 ---")
    model = iformer_s(img_size=img_h)
    print("模型建立成功。")

    # 在初始化時，模型內部會進行一次前向傳播來計算 width_list
    print(f"模型定義的 embed_dims: {model.embed_dims}")
    print(f"初始化後計算出的 width_list: {model.width_list}")

    # 準備測試輸入
    input_tensor = torch.rand(2, 3, img_h, img_w)
    print(f"\n--- 測試 iFormer-S 前向傳播 (輸入維度: {input_tensor.shape}) ---")
    ...
    try:
        ...
        print("前向傳播成功。")
        print("輸出是一個包含 4 個特徵圖的列表，維度如下:")
        for i, features in enumerate(output_features):
            print(f"  階段 {i+1}: {features.shape}")

        # 驗證 width_list 是否與實際運行的輸出通道數相符
        runtime_widths = [f.size(1) for f in output_features]
        print(f"\n實際運行的輸出特徵通道數: {runtime_widths}")
        
        assert model.width_list == runtime_widths, "width_list 與實際輸出不符!"
        assert model.width_list == model.embed_dims, "width_list 與 embed_dims 不符!"
        print("width_list 驗證成功。")
