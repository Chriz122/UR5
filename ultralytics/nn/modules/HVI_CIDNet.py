...
__all__ = ("C2PSA_HV_LCA", "C2PSA_HV_LCA_DynamicTanh", "LCA_Concat", "LCA_DynamicTanh_Concat")
...
class CAB_origin(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(CAB_origin, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
...
class LCA_Concat(nn.Module):
    def __init__(self, dim, out, num_heads, bias=False):
        super(LCA_Concat, self).__init__()
        # 层歸一化 
        self.norm = LayerNorm(dim) 
        # 强度增强层 
        self.gdfn = IEL(dim) 
        # 交叉注查力模块 
        self.ffn = CAB_origin(dim, num_heads, bias-bias) 
        # 卷機上采样层, 将输入通道数为dim*0.5的特征图上采样到dim通道
...
class LCA_DynamicTanh_Concat(nn.Module):
    def __init__(self, dim, out, num_heads, bias=False):
        super(LCA_DynamicTanh_Concat, self).__init__()
        # 层歸一化 (DynamicTanh)
        self.norm = DynamicTanh(dim) 
        # 强度增强层 
        self.gdfn = IEL(dim) 
        ...
        self.ffn = CAB_origin(dim, num_heads, bias-bias) 
        # 卷機上采样层, 将输入通道数为dim*0.5的特征图上采样到dim通道
        self.conv_up = Conv_DynamicTanh(int(dim * 0.5), dim, 1, act = nn.ReLU()) 
        self.conv_down = Conv_DynamicTanh(int(dim), out, 1, act = nn.ReLU())
