...
├── change_uvZtoXWYXZW.m      # 2D 眼到手校準腳本 (MATLAB)
├── change_uvZtoXWYXZW.py     # 2D 眼到手校準腳本 (Python)
├── eye_to_hand_calibration_new2.m # 3D 眼到手校準腳本 (MATLAB)
├── eye_to_hand_calibration_new2.py # 3D 眼到手校準腳本 (Python)
...
├── pose.txt                  # 姿態資訊
├── test.m                    # 內插, 測試腳本（讀取姿態並插值）(MATLAB)
├── test.py                   # 內插, 測試腳本（讀取姿態並插值）(Python)
```
...
- **功能**：
  - 讀取影像 (A*.png) 和深度資料 (D*.txt)。
  - 在影像中檢測棋盤格點。
  - 應用座標轉換將像素座標轉換為世界座標。
  - 輸出最小和最大 Z 值。
- **依賴項**：需要影像檔案 'A*.png' 和深度檔案 'D*.txt'。
